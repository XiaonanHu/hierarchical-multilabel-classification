...
class Trainer(object):
    ...
    def valid_epoch(self,data, epoch=None):
    ...
    def train_epoch(self,data,epoch=None):
        ...
        # print("\n------------- train result --------------")
        # # epoch metric
        # #self.outputs_tensor = torch.cat(self.outputs, dim =0).cpu().detach()
        # #self.targets_tensor = torch.cat(self.targets, dim =0).cpu().detach()
        # self.outputs = torch.cat(self.outputs, dim =0).cpu().detach()
        # self.targets = torch.cat(self.targets, dim =0).cpu().detach()
        # self.result['loss'] = tr_loss.avg
        # if self.epoch_metrics:
        #     for metric in self.epoch_metrics:
        #         #metric(logits=self.outputs_tensor, target=self.targets_tensor)
        #         metric(logits=self.outputs, target=self.targets)
        #         value = metric.value()
        #         if value:
...
#             break
        ...
        if len(train_data) != 0:
            train_log = self.train_epoch(train_data, epoch)
        else:
            train_log = None
        return train_log
        # if len(valid_data) != 0:
